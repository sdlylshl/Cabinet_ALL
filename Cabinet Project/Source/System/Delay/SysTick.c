...
/**
  ...
  * @retval None
  */
__STATIC_INLINE void SysTick_Init(uint32_t dely)
{
			// 1s钟执行SystemCoreClock个时钟周期   16 777 215
    // SystemFrequency / 1000    1ms中断一次(72 000 000)
	
		//SysTick_Config ---- inline core_cm3.h
    if (SysTick_Config(SystemCoreClock / dely)) 
    {
        /* Capture error */
        //TODO DEBUG
        return;
